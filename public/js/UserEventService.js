...
const server = http.createServer((req, res) => {
    ...
    if (req.method == 'GET') {
        ...
        getUserEvents(queryparams, function (response) { res.end(`${JSON.stringify(response)}`) })
        ...
        req.on('end', function () {
            ...
            postUserEvents(JSON.parse(body), function (response) { res.end(JSON.stringify(response)) })
        })
    }
        ...
        req.on('end', function () {
            console.log('Body: ' + body)
            res.writeHead(200, { 'Content-Type': 'application/json' })
            deleteUserEvents(JSON.parse(body), function (response) { res.end(JSON.stringify(response)) })
        })
    }
        ...
        req.on('end', function () {
            console.log('Body: ' + body)
            res.writeHead(200, { 'Content-Type': 'application/json' })
            putUserEvents(JSON.parse(body), function (response) { res.end(JSON.stringify(response)) })
        })
    }
});
            ...
            try {
                ...
                for (let i = 0; i < keys.length; i++) {
                    if (keys[i] !== "event_id" && keys[i] !== "table") {
                        res = await client.query({
                            text: `update ${table} set ${keys[i]} = '${queryparams[keys[i]]}' where eventid=$1`,
                            values: [event_id]
                        })
                    }
                }
