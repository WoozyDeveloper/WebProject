...
function addFloodMarkers() {
  ...
  fetch('https://localhost:4001?table=Floods')
    .then((response) => response.json())
    .then((data) => {
...
function addCities(start, end) {
  ...
  fetch(
    `https://localhost:4001/?table=Weather&starttime=${start}&endtime=${end}`
  )
    .then((response) => response.json())
    .then((data) => {
      for (let i = 0; i < data.length; i++) {
...
function addUserEarthquakes(start, end) {
  fetch(
    `https://localhost:4001?table=Earthquakes&starttime=${start}&endtime=${end}`
  )
    .then((response) => response.json())
    .then((data) => {
      for (let i = 0; i < data.length; i++) {
...
function showOtherEvents(queryparams) {
  ...
  fetch(
    `https://localhost:4003?start=${queryparams['start-date-event']}&end=${queryparams['end-date-event']}&urgency=${queryparams.urgency}&category=${queryparams.category}`
  )
    .then((response) => response.json())
    .then((data) => {
      let put = false;
