...
fetch(`https://localhost:4002/?table=users&email=${escapeHtml(usermail)}`)
...
function accountInfo() {
    ...
    changeFieldSend.onclick = async function (event) {
      ...
      try {
        const fetchResponse = await fetch(
          'https://localhost:4002/updateUsername',
          settings
        );
  ...
  changeMail.onclick = function (event) {
    ...
    changeFieldSend.onclick = async function (event) {
      ...
      try {
        const fetchResponse = await fetch(
          'https://localhost:4002/updateEmail',
          settings
        );
        const response = await fetchResponse.json();
        ...
        if (response.status === 'updated user') {
          ...
          changeDiv.appendChild(succesMessage);
          deleteCookie('token');
          window.location.href = 'https://localhost:3000/pages/login.html';
  ...
  changePassword.onclick = function (event) {
    ...
    changePasswordConfirm.onclick = async function (event) {
      if (
        ...
        try {
          const fetchResponse = await fetch(
            'https://localhost:4002/updatePassword',
            settings
          );
          const response = await fetchResponse.json();
...
function preferences() {
  ...
  removalPromise.then(
    fetch(
      `https://localhost:4002/?table=UserPreferences&userid=${escapeHtml(
        userid
      )}`
    )
      .then((response) => response.json())
  ...
  if (what === 'preference') {
    ...
    addButton.onclick = function (event) {
      ...
      submit.onclick = async function (event) {
        ...
        console.log('intru sa fac post macar?');
        try {
          const fetchResponse = await fetch('https://localhost:4002', settings);
          const response = await fetchResponse.json();
          console.log(response);
