...
fetch(`http://157.230.100.116:4002/?table=users&email=${escapeHtml(usermail)}`)
...
function accountInfo() {
    ...
    changeFieldSend.onclick = async function (event) {
      ...
      try {
        const fetchResponse = await fetch(
          'http://157.230.100.116:4002/updateUsername',
          settings
        );
  ...
  changeMail.onclick = function (event) {
    ...
    changeFieldSend.onclick = async function (event) {
      ...
      try {
        const fetchResponse = await fetch(
          'http://157.230.100.116:4002/updateEmail',
          settings
        );
        const response = await fetchResponse.json();
        ...
        if (response.status === 'updated user') {
          ...
          changeDiv.appendChild(succesMessage);
          deleteCookie('token');
          window.location.href = 'http://157.230.100.116:3000/pages/login.html';
  ...
  changePassword.onclick = function (event) {
    ...
    changePasswordConfirm.onclick = async function (event) {
      if (
        ...
        try {
          const fetchResponse = await fetch(
            'http://157.230.100.116:4002/updatePassword',
            settings
          );
          const response = await fetchResponse.json();
...
function preferences() {
  ...
  removalPromise.then(
    fetch(
      `http://157.230.100.116:4002/?table=UserPreferences&userid=${escapeHtml(
        userid
      )}`
    )
      .then((response) => response.json())
